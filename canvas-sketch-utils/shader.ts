...
export default function createShader(opt: any) {
  opt = opt || {};
  if (!opt.gl) {
    throw new Error(
  ...
  if (typeof clearColor === "string") {
    ...
    if (!parsed.rgb) {
      throw new Error(
        'Error parsing { clearColor } color string "' + clearColor + '"'
      );
    }
    ...
    clearColor &&
    (!Array.isArray(clearColor) || clearColor.length < 3)
  ) {
    throw new Error(
...
function isTextureLike(data: any) {
