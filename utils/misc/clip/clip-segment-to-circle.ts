...
export default function intersectSegmentCircle(
  p0: number[],
  p1: number[],
  circle: number[],
  circleRadius: number,
...
function intersect(
  ...
  if (p1InCircle || p2InCircle) {
    ...
    var hitB = p1InCircle
      ? projectPoint(px, py, x2, y2, a)
      : projectPoint(px, py, x1, y1, a);
  ...
  if (
    (almostEqual(x1, px) && almostEqual(y1, py)) ||
...
function layDistance(x1: number, y1: number, x2: number, y2: number) {
  var dx = x2 - x1;
  var dy = y2 - y1;
  return dx * dx + dy * dy;
}
...
function projectPoint(
  ...
  return [srcx + t * (destx - srcx), srcy + t * (desty - srcy)];
}

function closestPointOnLineFromPoint(
  x1: number,
  y1: number,
  x2: number,
  y2: number,
  ...
  var c2 = vx * vx + vy * vy;
  var ratio = c1 / c2;
  return [x1 + ratio * vx, y1 + ratio * vy];
}

function closestPointOnSegmentFromPoint(
  x1: number,
  y1: number,
  x2: number,
  y2: number,
